...
// определение маштаба максисального и минаимального
    ///
            massiv:=nil;
    SetLength(massiv,5);
...
begin
  ...
  Form_DBTargetsAppendAndEdit.Caption:='Создание новой записи';
...
var
  isDelete: boolean;
begin
  if MessageDlg('Вы уверены, что хотите удалить все записи?', mtConfirmation, [mbYes, mbNo], 0)=mrYes then
  begin
...
begin
  ...
  begin
    ...
    if (isDeleteSTATISTICS=false) or (isDeleteTARGSIND=false) then
      MessageDlg('Ошибка в процессе удаления записей.', mtError, [mbOK], 0);
    Form_DataBase.Btn_DBRefresh.OnClick(Sender);
...
var
  ...
  SaveFile: TextFile;
begin
  SaveDlg_ChTPSaveToFile.FileName:='Список целей';
  ...
  begin
    try
      ...
      begin
        MessageDlg('Ошибка при сохранении данных по целям в файл.', mtError, [mbOk], 0);
       ...
       //     SumRm:=0;
        ...
        DBForm.Targets.Next;
         ...
         //    Edit2.Text:=FloatToStr(D);
            ...
            yBegin:=round(100*(M-S)+0.5)/100;// Начало интервала отбраковки
         //                 Edit4.Text:=FloatToStr( yBegin);
            yEnd:=round(100*(S+M)+0.5)/100;  // Конец инткервала отбраковки
...
begin
  ...
  Form_DBTargetsAppendAndEdit.Caption:='Редактирование записи';
  Form_DBTargetsAppendAndEdit.Show;
end;
...
begin
  Form_Main.Label_OpenCloseTargets.Caption:='Д';
...
begin
  ...
  Form_Main.Label_OpenCloseTargets.Caption:='З';
  Form_Main.BitBtn_TargetsMenu.Glyph:=nil;
  Application.ProcessMessages();
